<?
	...
	$pdf->AliasNbPages();
  	...
  	$pdf->AddPage();
   	...
   	$pdf->Cell(19, 1, 'Sales Summary Monthly Report', 0, 0, 'C', 0);
   	$pdf->Ln();
   	$pdf->Ln();

   	$pdf->SetFont('Courier', 'B', 10);
   	$pdf->Cell(4, 0.4, 'Month / Year', 0, 0, 'L', 0);
   	$pdf->Cell(4, 0.4, ': '.mon_str($param1)." ".$param2, 0, 0, 'L', 0);
   	$pdf->Ln();
   	$pdf->Ln();

	$qry = "";
	if (!empty($param3)) {
		$db1->Query("SELECT desc FROM tags WHERE tagid = $param3");
		list($desc) = $db1->Row();
		$qry = "AND c.tagid = ".$param3;
	...
	if (!empty($param4)) {
		$qry .= " AND k.cashier = '$param4'";
   		$pdf->Cell(4, 0.4, 'Cashier', 0, 0, 'L', 0);
   		$pdf->Cell(4, 0.4, ': '.$param4, 0, 0, 'L', 0);
	   	$pdf->Ln();
	}
   	...
   	$db1->Query("SELECT m.menuid,m.name,m.price,SUM(d.qty) AS sqty,(m.price * SUM(d.qty)) AS total FROM menus m,menucats c,dtlkot d,receipt k WHERE d.menuid = m.menuid AND m.catid = c.catid AND k.kotid = d.kotid AND MONTH(k.`date`) = $param1 AND YEAR(k.`date`) = $param2 AND d.is_void = 'N' $qry GROUP BY m.menuid ORDER BY sqty DESC");
   	if ($db1->NRow() > 0) {
      	$i = 1;
