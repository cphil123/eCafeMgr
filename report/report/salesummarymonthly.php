<?
	...
	if (!empty($param3)) {
	   	...
	   	$pdf->Cell(14, 1, 'Revenue Summary Monthly Report', 0, 0, 'C', 0);
	   	$pdf->Ln();
	   	$pdf->SetFont('Courier', 'B', 8);
	   	$pdf->Cell(2, 0.4, 'Month / Year', 0, 0, 'L', 0);
	   	...
	   	$db1->Query("SELECT * FROM salesummary WHERE MONTH(`date`) = $param1 AND YEAR(`date`) = $param2 AND cashier = '$param3'");
	   	if ($db1->NRow() > 0) {
	      	$i = 1;
			...
			$rev = 0;
	      	...
	      	$pdf->SetFont('Courier', 'B', 8);
	       	$pdf->Cell(9, 0.5, 'Revenue', 1, 0, 'R', 1);
	   	...
	   	$pdf->SetFont('Courier', 'B', 12);
	   	$pdf->Cell(16, 1, 'Sales Summary Monthly Report', 0, 0, 'C', 0);
	   	$pdf->Ln();
	   	$pdf->Ln();
	   	$pdf->SetFont('Courier', 'B', 8);
	   	$pdf->Cell(2, 0.4, 'Month / Year', 0, 0, 'L', 0);
	   	...
	   	$pdf->Cell(2, 0.6, 'Total Cash', 1, 0, 'C', 1);
	   	$pdf->Cell(2, 0.6, 'Discount', 1, 0, 'C', 1);
	   	$pdf->Cell(2, 0.6, 'Total Revenue', 1, 0, 'C', 1);
	   	$pdf->Ln();
		
	   	$db1->Query("SELECT * FROM salesummary WHERE MONTH(`date`) = $param1 AND YEAR(`date`) = $param2");
	   	if ($db1->NRow() > 0) {
	      	$i = 1;
	      	$pdf->SetFont('Courier', '', 8);
	      	$pdf->SetFillColor(255, 255, 255);
